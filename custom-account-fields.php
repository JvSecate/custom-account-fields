...
// Translations, some comments and some strings were generated by LLMs
...
JS;
    ...
    private function render_field_input(array $field, string $field_id, $value, bool $required, string $class): void {
        ...
        if ($type === 'textarea') {
            echo '<textarea class="' . esc_attr($class) . '" name="' . esc_attr($key) . '" id="' . esc_attr($field_id) . '" placeholder="' . esc_attr((string) $field['placeholder']) . '" autocomplete="' . esc_attr((string) $field['autocomplete']) . '"' . ($required ? ' required' : '') . '>' . esc_textarea((string) $value) . '</textarea>';
            return;
        }

        if ($type === 'select') {
            echo '<select class="' . esc_attr($class) . '" name="' . esc_attr($key) . '" id="' . esc_attr($field_id) . '"' . ($required ? ' required' : '') . '>';
        ...
        if ($type === 'checkbox') {
            echo '<label><input type="checkbox" name="' . esc_attr($key) . '" id="' . esc_attr($field_id) . '" value="1" ' . checked((string) $value, '1', false) . ' /> ' . esc_html((string) $field['label']) . '</label>';
            return;
        }

        $html_type = in_array($type, ['email', 'tel', 'number', 'date'], true) ? $type : 'text';
        echo '<input type="' . esc_attr($html_type) . '" class="' . esc_attr($class) . '" name="' . esc_attr($key) . '" id="' . esc_attr($field_id) . '" value="' . esc_attr((string) $value) . '" placeholder="' . esc_attr((string) $field['placeholder']) . '" autocomplete="' . esc_attr((string) $field['autocomplete']) . '"' . ($required ? ' required' : '') . ' />';
    }
